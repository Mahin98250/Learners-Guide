...
async function getCaller(admin: ReturnType<typeof createClient>, token: string) {
  ...
  return { id: data.user.id, role: String(profile.user?.app_metadata?.role || ""), ref: String(profile.user?.app_metadata?.ref || "") };
}
...
async function authorizePath(admin: ReturnType<typeof createClient>, bucket: string, path: string, caller: { role: string; ref: string }) {
  ...
  const { data, error } = await admin.from(table).select("id,tid").eq("storage_path", path).limit(10);
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!isPdfPath(path)) return json({ status: "skipped", reason: "format-not-supported", originalSize: null, optimizedSize: null });
    ...
    if (originalSize < 512 * 1024) return json({ status: "skipped", reason: "already-small", originalSize, optimizedSize: originalSize });
    const source = await PDFDocument.load(originalBytes, { ignoreEncryption: false });
    const pageCount = source.getPageCount();
    const optimizedBytes = await source.save({ useObjectStreams: true, addDefaultPage: false, updateFieldAppearances: false, objectsPerTick: 50 });
    if (optimizedBytes.byteLength >= originalSize) return json({ status: "unchanged", reason: "optimized-version-not-smaller", originalSize, optimizedSize: originalSize, pageCount });
    const check = await PDFDocument.load(optimizedBytes, { ignoreEncryption: false });
    if (check.getPageCount() !== pageCount) return json({ status: "rejected", reason: "validation-page-count-mismatch", originalSize, optimizedSize: originalSize, pageCount });
    const replacement = new Blob([optimizedBytes], { type: "application/pdf" });
    const { error: uploadError } = await admin.storage.from(bucket).upload(path, replacement, { upsert: true, contentType: "application/pdf", cacheControl: "3600" });
    ...
    return json({ status: "optimized", originalSize, optimizedSize: optimizedBytes.byteLength, savedBytes: originalSize - optimizedBytes.byteLength, savingsPercent: Number((((originalSize - optimizedBytes.byteLength) / originalSize) * 100).toFixed(2)), pageCount, profile: "safe-structural-pdf-v1" });
  } catch (error) {
