...
function formatError(error: unknown): string {
  if (error instanceof Error) {
    ...
    const parts = fields.filter((field) => details[field] !== undefined && details[field] !== null).map((field) => `${field}=${String(details[field])}`);
    return parts.length > 0 ? parts.join("; ") : error.message;
  }
  if (typeof error === "object" && error !== null) {
    try {
      const safe = JSON.parse(JSON.stringify(error, (key, value) => ["password", "url", "connectionString"].includes(key) ? "[redacted]" : value));
...
async function authorizeGitHub(req: Request) {
  ...
  const { payload } = await jwtVerify(token, githubJWKS, { issuer: GITHUB_ISSUER, audience: OIDC_AUDIENCE, algorithms: ["RS256"] });
...
function parseConnectionString(input: string) {
  ...
  return { password: decodeURIComponent(parsed.password), database: decodeURIComponent(parsed.pathname.replace(/^\//, "") || "postgres"), hostname: parsed.hostname, port: parsed.port ? Number(parsed.port) : 5432 };
}

async function verifyDatabaseUrl(url: string) {
  const client = postgres(url, { max: 1, idle_timeout: 3, connect_timeout: 10, ssl: "require", prepare: false });
  try {
...
async function getVerifiedSessionPoolerUrl() {
  ...
  const hosts = [configuredHost, DEFAULT_POOLER_HOST, `aws-${REGION}.pooler.supabase.com`, `aws-0-${REGION}.pooler.supabase.com`].filter((host, index, list): host is string => Boolean(host) && list.indexOf(host) === index);
  if (source.port === 5432 && source.hostname.endsWith("pooler.supabase.com")) return { url: sourceDbUrl, verification: await verifyDatabaseUrl(sourceDbUrl!) };
  ...
  for (const host of hosts) {
    ...
    try { return { url: candidate, verification: await verifyDatabaseUrl(candidate) }; }
    catch (error) { failures.push(`${host}: ${formatError(error)}`); }
  }
...
async function collectStorage() {
  ...
  for (const bucket of buckets) {
    ...
    const limit = 1000;
    while (true) {
      const result = await admin.from("storage.objects").select("bucket_id,name,id,metadata,created_at,updated_at,owner").eq("bucket_id", bucket.id).order("name", { ascending: true }).range(offset, offset + limit - 1);
      if (result.error) throw result.error;
      const page = result.data || [];
      for (const object of page) {
        const signed = await admin.storage.from(bucket.id).createSignedUrl(object.name, 3600);
        if (signed.error || !signed.data?.signedUrl) throw signed.error || new Error(`failed to create signed URL for ${bucket.id}/${object.name}`);
        objects.push({ bucket: bucket.id, name: object.name, id: object.id, metadata: object.metadata, created_at: object.created_at, updated_at: object.updated_at, owner: object.owner, signed_url: signed.data.signedUrl });
      }
...
async function collectAuthUsers() {
  ...
  while (true) {
    ...
    users.push(...batch.map((user) => ({ id: user.id, aud: user.aud, role: user.role, email: user.email, phone: user.phone, created_at: user.created_at, updated_at: user.updated_at, email_confirmed_at: user.email_confirmed_at, phone_confirmed_at: user.phone_confirmed_at, last_sign_in_at: user.last_sign_in_at, app_metadata: user.app_metadata, user_metadata: user.user_metadata, identities: user.identities, is_anonymous: user.is_anonymous })));
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (mode === "storage") return responseJson({ ok: true, project_ref: PROJECT_REF, storage, auth_users: authUsers, issued_at: new Date().toISOString() });
    const db = await getVerifiedSessionPoolerUrl();
