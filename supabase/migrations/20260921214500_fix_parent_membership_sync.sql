-- Multi-institute Phase 4 fix: keep parent/student links tenant-scoped even when a legacy
-- service-role provisioner previously created the link without institute_id.

update public.parent_student_links psl
set institute_id = s.institute_id
from public.students s
where psl.student_id = s.id
  and psl.institute_id is null
...
begin
  ...
  if (
    not exists (
      select 1 from public.institute_role_permissions rp
