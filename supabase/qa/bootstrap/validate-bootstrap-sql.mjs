...
const tableCount = (normalized.match(/\bcreate\s+table\s+(?:"public"|public)\./g) ?? []).length;
if (tableCount !== 27) fail(`Expected exactly 27 public application tables, found ${tableCount}.`);

const functionMatches = [...sql.matchAll(/\bcreate\s+or\s+replace\s+function\s+(?:"public"|public)\.([\w"]+)\s*\(/gi)];
