...
export function AdminManagementHub({ user, onOpenLegacy, onBack, onLogout }: Props) {
  ...
  return <div className="admin-management-hub">
    ...
    <header className="amh-top"><div className="amh-brand"><div className="amh-logo"><LGLogo size={32} showText={false} /></div><div><b>Learner's Guide</b><small>Admin Management</small></div></div><button type="button" className="amh-btn amh-back" onClick={onBack}>← Dashboard</button><button type="button" className="amh-btn amh-logout" onClick={onLogout}>Logout</button></header>
