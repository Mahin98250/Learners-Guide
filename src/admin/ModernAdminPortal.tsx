...
const GROUPS: Group[] = [
  ...
  {
    label: "Insights",
    items: [{ key: "People & Analytics", icon: "↗", label: "Analytics", special: "analytics" }],
  },
];
...
export function ModernAdminPortal({ user, onLogout }: { user: AdminUser; onLogout: () => void }) {
  ...
  return (
    <div className="modern-admin">
      ...
      <button
        type="button"
        className="modern-admin-mobile-back"
        onClick={() => {
          if (mobileOpen) setMobileOpen(false);
          else choose(allItems[0]);
        }}
        aria-label={mobileOpen ? "Close navigation" : "Back to dashboard"}
      >
        {mobileOpen ? "×" : "←"}
      </button>
      ...
      <aside className={`modern-admin-sidebar ${mobileOpen ? "open" : ""}`}>
        <div className="modern-admin-brand">
          <div className="modern-admin-brand-mark"><LGLogo size={58} showText={false} /></div>
          <div>
            <strong>Learner's Guide</strong>
            <span>Admin Portal</span>
          </div>
        </div>

        <div className="modern-admin-profile">
          <div className="modern-admin-avatar">{(user.name || "A").trim().charAt(0).toUpperCase()}</div>
          <div className="modern-admin-profile-copy">
            <strong>{user.name || "Admin"}</strong>
            <span>Administrator</span>
          </div>
          <span className="modern-admin-online" title="Signed in" />
        </div>

        <nav className="modern-admin-nav" aria-label="Admin navigation">
          {GROUPS.map((group) => (
            <div className="modern-admin-nav-group" key={group.label}>
              <div className="modern-admin-nav-label">{group.label}</div>
              {group.items.map((item) => (
                <button
                  type="button"
                  key={item.key}
                  className={`modern-admin-nav-item ${active === item.key ? "active" : ""}`}
                  onClick={() => choose(item)}
                >
                  <span className="modern-admin-nav-icon" aria-hidden="true">{item.icon}</span>
        ...
        <div className="modern-admin-sidebar-bottom">
          <div className="modern-admin-current-admin">
      ...
      <main className="modern-admin-main">
        <header className="modern-admin-topbar">
          <div className="modern-admin-heading">
            <span className="modern-admin-breadcrumb">Learner's Guide <b>•</b> Admin</span>
            <h1>{activeItem.label}</h1>
          </div>
          <div className="modern-admin-top-actions">
            <div className="modern-admin-top-admin">
              <div className="modern-admin-avatar small">{(user.name || "A").trim().charAt(0).toUpperCase()}</div>
              <div><strong>{user.name || "Admin"}</strong><span>Administrator</span></div>
            </div>
            <button type="button" className="modern-admin-top-logout" onClick={onLogout}>Logout</button>
          </div>
        </header>
        ...
        <section className="modern-admin-content">
          ...
          ) : activeItem.special === "leave" ? (
            <div className="modern-admin-special modern-admin-leave"><LeaveRequests user={user} student={null} canReview /></div>
          ) : (
            <div className={`modern-admin-legacy-host ${legacyReady ? "ready" : "loading"}`}>
              {!legacyReady && <div className="modern-admin-page-loading"><div className="modern-admin-spinner" /><span>Loading {activeItem.label}…</span></div>}
