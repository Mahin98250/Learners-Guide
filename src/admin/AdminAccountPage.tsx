...
function SecurityPage() {
  ...
  useEffect(() => { load(); }, []);
  const sendRecovery = async () => { setNotice(""); setError(""); const target = dataEmail(email); if (!target) { setError("No verified email address is available for this account."); return; } const { data, error: recoveryError } = await supabase.functions.invoke("password-recovery-request", { body: { role: "admin", identifier: target } }); if (recoveryError) setError(recoveryError.message || "Unable to start password recovery."); else setNotice(data?.message || "If password recovery is configured for this account, a recovery message has been requested."); };
...
function CreateAdminPage() {
  ...
  const submit = async (event: FormEvent) => { event.preventDefault(); setError(""); setSuccess(""); if (!name.trim() || !loginEmail.trim()) { setError("Name and login email are required."); return; } if (!dataEmail(loginEmail)) { setError("Enter a valid administrator email address."); return; } if (password.length < 8) { setError("Password must be at least 8 characters."); return; } if (password !== confirm) { setError("Passwords do not match."); return; } setLoading(true); const { data, error: invokeError } = await supabase.functions.invoke("admin-provision-user", { body: { action: "create", role: "admin", loginId: loginEmail.trim(), password, name: name.trim() } }); if (invokeError) setError(invokeError.message || "Unable to create administrator account."); else if (data?.error) setError(data.error); else { setSuccess(`Administrator account created successfully. Login: ${data?.email || loginEmail.trim()}`); setName(""); setLoginEmail(""); setPassword(""); setConfirm(""); } setLoading(false); };
