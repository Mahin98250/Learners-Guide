...
function testDateKey(test: Row) {
  return String(test.test_date || "").slice(0, 10);
}
...
export default function TestManagementPage() {
  ...
  const [form, setForm] = useState({ title: "", description: "", batch_id: "", subject: "", test_date: todayKey(), total_marks: "100" });
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setTests((ts as Row[]).sort((a, b) => testDateKey(b).localeCompare(testDateKey(a))));
  ...
  const saveTest = async () => {
    ...
    try {
      ...
      setForm({ title: "", description: "", batch_id: "", subject: "", test_date: todayKey(), total_marks: "100" });
      await load();
  ...
  const upcomingTests = useMemo(() => tests.filter(test => testDateKey(test) >= today).sort((a, b) => testDateKey(a).localeCompare(testDateKey(b))), [tests, today]);
  const previousTests = useMemo(() => tests.filter(test => testDateKey(test) < today).sort((a, b) => testDateKey(b).localeCompare(testDateKey(a))), [tests, today]);

  const renderTestsTable = (items: Row[], emptyText: string) => loading ? <p>Loading…</p> : items.length === 0 ? <p style={{ color: "#64748B", margin: 0 }}>{emptyText}</p> : <div style={{ overflowX: "auto" }}><table style={{ width: "100%", borderCollapse: "collapse" }}><thead><tr>{["Test", "Batch", "Subject", "Date", "Total", "Status", "Action"].map(x => <th key={x} style={{ textAlign: "left", padding: 10, borderBottom: "1px solid #E2E8F0", whiteSpace: "nowrap" }}>{x}</th>)}</tr></thead><tbody>{items.map(t => { const b = batches.find(x => String(x.id) === String(t.batch_id)); return <tr key={String(t.id)}><td style={{ padding: 10, fontWeight: 700 }}>{t.title}</td><td style={{ padding: 10 }}>{b?.name || t.batch_id}</td><td style={{ padding: 10 }}>{t.subject}</td><td style={{ padding: 10, whiteSpace: "nowrap" }}>{testDateKey(t)}</td><td style={{ padding: 10 }}>{t.total_marks}</td><td style={{ padding: 10 }}>{t.status}</td><td style={{ padding: 10 }}><button onClick={() => void openResults(t)} style={btn("#8B5CF6")}>{testDateKey(t) < today ? "View / Enter Results" : "Enter Results"}</button></td></tr>; })}</tbody></table></div>;

  return <div style={{ minHeight: "100%", padding: 22, background: "#F0F4FF", color: "#0F1B3D" }}>
    <div style={{ marginBottom: 18 }}><h1 style={{ margin: 0, fontSize: 24 }}>Tests & Results</h1><p style={{ margin: "6px 0", color: "#64748B" }}>Create tests for a batch, notify students, and manage results student-by-student.</p></div>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(150px,1fr))", gap: 12, marginBottom: 18 }}>
      <div style={{ ...card, padding: 16 }}><div style={{ fontSize: 12, color: "#64748B" }}>Upcoming Tests</div><div style={{ fontSize: 28, fontWeight: 900, marginTop: 4 }}>{upcomingTests.length}</div></div>
      <div style={{ ...card, padding: 16 }}><div style={{ fontSize: 12, color: "#64748B" }}>Previous Tests</div><div style={{ fontSize: 28, fontWeight: 900, marginTop: 4 }}>{previousTests.length}</div></div>
      <div style={{ ...card, padding: 16 }}><div style={{ fontSize: 12, color: "#64748B" }}>Total Tests</div><div style={{ fontSize: 28, fontWeight: 900, marginTop: 4 }}>{tests.length}</div></div>
    </div>
