...
export function AdminDashboardAnalytics({ onBack }: { onBack?: () => void }) {
  ...
  useEffect(() => { let live = true; (async () => { setLoading(true); if (!instituteId) { setError("An active institute workspace must be selected."); setLoading(false); return; } const results = await Promise.all([supabase.from("students").eq("institute_id", instituteId).select(DASHBOARD_SELECTS.students).eq("institute_id", instituteId), supabase.from("teachers").eq("institute_id", instituteId).select(DASHBOARD_SELECTS.teachers).eq("institute_id", instituteId), supabase.from("attendance").eq("institute_id", instituteId).select(DASHBOARD_SELECTS.attendance).eq("institute_id", instituteId), supabase.from("fees").eq("institute_id", instituteId).select(DASHBOARD_SELECTS.fees).eq("institute_id", instituteId), supabase.from("homework").eq("institute_id", instituteId).select(DASHBOARD_SELECTS.homework).eq("institute_id", instituteId), supabase.from("announcements").select(DASHBOARD_SELECTS.announcements).eq("institute_id", instituteId).order("created_at", { ascending: false })]); const firstError = results.find((r) => r.error)?.error; if (!live) return; if (firstError) setError(firstError.message); else { setStudents(results[0].data || []); setTeachers(results[1].data || []); setAttendance(results[2].data || []); setFees(results[3].data || []); setHomework(results[4].data || []); setAnnouncements(results[5].data || []); } setLoading(false); })(); return () => { live = false; }; }, []);
...
export function AdminProfileAnalytics({ onBack }: { onBack?: () => void }) {
  ...
  useEffect(() => { let live = true; (async () => { if (!instituteId) { setError("An active institute workspace must be selected."); setLoading(false); return; } const [s, t] = await Promise.all([supabase.from("students").eq("institute_id", instituteId).select(PROFILE_SELECTS.students).eq("institute_id", instituteId), supabase.from("teachers").eq("institute_id", instituteId).select(PROFILE_SELECTS.teachers).eq("institute_id", instituteId)]); if (!live) return; const e = s.error || t.error; if (e) setError(e.message); setStudents(s.data || []); setTeachers(t.data || []); setLoading(false); })(); return () => { live = false; }; }, []);
