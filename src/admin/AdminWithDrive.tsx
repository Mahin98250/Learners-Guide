...
export function AdminWithDrive({ user, onLogout }: { user: AdminUser; onLogout: () => void }) {
  ...
  useEffect(() => {
    const mountAdminNavigation = () => {
      ...
      const materialNav = Array.from(nav.querySelectorAll<HTMLElement>(".nav")).find((item) =>
        (item.textContent || "").trim().includes("Study Materials"),
      );
      if (materialNav && !materialNav.dataset.driveBound) {
        materialNav.dataset.driveBound = "true";
        materialNav.addEventListener("click", (event) => {
  ...
  if (materialsOpen) {
    return (
      <div style={{ minHeight: "100vh", background: "#F0F4FF" }}>
        ...
        <MaterialsDrive onClose={() => setMaterialsOpen(false)} />
      </div>
    );
  }
  ...
  return (
    <>
      ...
      <style>{`@media(max-width:900px){.mobile-admin-logout{display:block!important}.admin-leave-nav{min-width:130px!important;white-space:nowrap!important;color:#0F1B3D!important}.admin-leave-nav:hover{background:#F0F4FF!important}}@media(min-width:901px){.admin-leave-nav:hover{background:#ffffff12!important}.admin-leave-nav{color:#ffffff8c!important}.admin-leave-nav:focus-visible{outline:2px solid #4361EE;outline-offset:2px}}`}</style>
