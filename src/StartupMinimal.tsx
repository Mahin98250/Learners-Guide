...
export default function StartupMinimal() {
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => setExiting(true), 140);
  ...
  return (
    <div className={`lg-minimal-launch${exiting ? " lg-minimal-launch--exit" : ""}`} aria-hidden="true">
      ...
      <style>{`
        .lg-minimal-launch{position:fixed;inset:0;z-index:2147483647;pointer-events:none;background:#fff;display:flex;align-items:center;justify-content:center;opacity:1;overflow:hidden;transition:opacity 140ms cubic-bezier(.22,1,.36,1)}
