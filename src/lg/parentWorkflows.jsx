...
export function ParentApp({ user, onLogout }) {
  ...
  const [attendance, setAttendance] = useState([]), [fees, setFees] = useState([]), [tests, setTests] = useState([]), [results, setResults] = useState([]);
  const [homework, setHomework] = useState([]), [materials, setMaterials] = useState([]), [memberships, setMemberships] = useState([]), [timetable, setTimetable] = useState([]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!ids.length) { setChildren([]); setMemberships([]); setAttendance([]); setFees([]); setTests([]); setResults([]); setHomework([]); setMaterials([]); setTimetable([]); return; }
      ...
      setResults(resultResponse.data || []); setMaterials(materialResponse.data || []);
  ...
  const moreItems = [
    { key: "timetable", icon: "🗓️", label: "Classes", hint: "Today's schedule" },
    { key: "homework", icon: "📝", label: "Homework", hint: `${childHomework.length} assignments` },
    { key: "tests", icon: "📋", label: "Tests", hint: `${childTests.length} scheduled` },
    { key: "results", icon: "🏆", label: "Results", hint: `${childResults.length} recorded` },
    { key: "materials", icon: "📚", label: "Materials", hint: `${childMaterials.length} resources` },
    { key: "leave", icon: "🏖️", label: "Leave", hint: "Request or track leave" },
  ];
  ...
  const content = loading ? <Card>Loading your child's data…</Card> : error ? <Card><div style={{ color: "#c0392b", marginBottom: 10 }}>{error}</div><button onClick={() => void load()}>Try again</button></Card> : !selected ? <Card>No student is linked to this parent account.</Card> : (
    <main className="parent-portal">
      ...
      {tab === "home" && <><div className="pp-section"><strong>Quick access</strong><span>Simple overview</span></div><div className="pp-home-actions"><button className="pp-home-action" onClick={() => setTab("analytics")}><span>📊</span><b>Student analytics</b><small>Progress, attendance and results</small></button><button className="pp-home-action" onClick={() => setTab("attendance")}><span>✅</span><b>Attendance</b><small>{childAttendance.length} recorded days</small></button><button className="pp-home-action" onClick={() => setTab("fees")}><span>₹</span><b>Fees</b><small>{childFees.length} records</small></button><button className="pp-home-action" onClick={() => setTab("more")}><span>☰</span><b>More sections</b><small>Classes, homework, tests & more</small></button></div></>}
      ...
      {tab === "leave" && <LeaveRequests user={user} student={selected} />}
      {tab === "more" && <><div className="pp-section"><strong>More</strong><span>Everything else, neatly organised</span></div><div className="pp-more-grid">{moreItems.map((item) => <button key={item.key} className="pp-more-card" onClick={() => setTab(item.key)}><div className="pp-more-icon">{item.icon}</div><b>{item.label}</b><small>{item.hint}</small></button>)}</div></>}
    </main>
  );
