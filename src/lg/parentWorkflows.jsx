...
export function ParentApp({ user, onLogout }) {
  ...
  const content = loading ? <Card style={{ padding: 24, textAlign: "center", color: "#747c94" }}>Loading your child's dashboard…</Card> : error ?
    ...
    <main className="parent-portal" style={{ width: "100%", maxWidth: "100%", minWidth: 0, boxSizing: "border-box" }}>
      ...
      {tab === "fees" && <><Sec title="Fees" />{sectionLoading ? SECTION_FALLBACK : <div className="pp-panel">{childFees.length ? childFees.map(f => <div style={row} key={f.id}><div><div className="pp-title">{f.desc || "Fee"}</div><div className="pp-sub">Due {f.due || "—"} · {f.status || "—"}</div></div><b>₹{Number(f.amount || 0).toLocaleString("en-IN")}</b></div>) : <div className="pp-note">No fee records found.</div>}</div>}</>}
    </main>;
