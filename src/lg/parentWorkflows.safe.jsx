...
export function ParentApp({ user, onLogout }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [tr, ar, fr, hw, tests] = await Promise.all([
      ...
      const testIds = (tests.data || []).map((x) => String(x.id)).filter(Boolean);
  ...
  return <Shell header={<AppBar name={user.name} role="parent" userId={user.id} onLogout={onLogout} onNotif={() => setShowNotif(true)} />} tabs={tabs} activeTab={tab} setTab={setTab}>{content}</Shell>;
}
