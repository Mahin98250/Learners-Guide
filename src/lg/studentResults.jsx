...
export function STExams({ student }) {
  ...
  /*
   * The database can expose one assessment through both `examschedule` and
   * `tests`. Student-facing cards must represent the real assessment once.
   * Prefer the `tests` row because it is the batch-scoped source used for
   * student results; keep a scheduled exam only when no matching test exists.
   * A second pass also protects the UI if duplicate test rows are returned.
   */
  const testKeys=new Set(tests.map(t=>assessmentKey(t)));
  const uniqueSchedule=[];
  const seenSchedule=new Set();
  for(const exam of schedule){
    const key=assessmentKey(exam);
    if(testKeys.has(key)||seenSchedule.has(key))continue;
    seenSchedule.add(key);
    uniqueSchedule.push(exam);
  }
  const uniqueTests=[];
  const seenTests=new Set();
  for(const test of tests){
    const key=assessmentKey(test);
    if(seenTests.has(key))continue;
    seenTests.add(key);
    uniqueTests.push(test);
  }
  const rows=[...uniqueSchedule.map(e=>({kind:"schedule",id:`schedule-${e.id}`,title:e.title||"Exam",subject:e.subject,date:e.date,total:e.totalMarks??e.totalmarks,time:`${e.startTime||e.starttime||"—"}${e.endTime||e.endtime?` – ${e.endTime||e.endtime}`:""}`,venue:e.venue,syllabus:e.syllabus})),...uniqueTests.map(t=>({kind:"test",id:`test-${t.id}`,title:t.title||"Test",subject:t.subject,date:t.test_date,total:t.total_marks,time:"",venue:"",syllabus:t.description,status:t.status}))].sort((a,b)=>String(a.date||"").localeCompare(String(b.date||"")));
