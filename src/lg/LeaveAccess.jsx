...
function findBottomNav(){
 ...
 const matches=candidates.filter(el=>{const s=getComputedStyle(el);const r=el.getBoundingClientRect();return s.position==="fixed"&&r.bottom<=3&&r.width>280&&r.height>=55&&r.height<=140&&r.top>window.innerHeight-180});
...
function mountNavItem(onOpen){
 ...
 const mount=()=>{const nav=findBottomNav();if(!nav||nav.querySelector('[data-lg-leave-nav="true"]'))return;button=document.createElement("button");button.type="button";button.setAttribute("data-lg-leave-nav","true");button.setAttribute("aria-label","Open leave");button.innerHTML='<span style="display:block;font-size:22px;line-height:1.15">🏖️</span><span style="display:block;font-size:11px;font-weight:700;margin-top:3px">Leave</span>';button.style.cssText="flex:1 1 0;min-width:0;border:0;background:transparent;color:#64748b;padding:7px 3px calc(7px + env(safe-area-inset-bottom,0px));font:inherit;text-align:center;cursor:pointer;touch-action:manipulation;box-sizing:border-box";button.addEventListener("click",onOpen);nav.appendChild(button)};
 mount();observer=new MutationObserver(mount);observer.observe(document.body,{childList:true,subtree:true});return()=>{observer?.disconnect();if(button){button.removeEventListener("click",onOpen);button.remove()}};
}
export function LeaveAccess({user,student,canReview=false}){
 ...
 useEffect(()=>{let live=true;(async()=>{if(student||canReview||user?.role!=="parent")return;try{const{data:links}=await supabase.from("parent_student_links").select("student_id,status").eq("parent_auth_id",user.id).eq("status","active").limit(20);const ids=[...new Set((links||[]).map(x=>String(x.student_id)).filter(Boolean))];if(!ids.length)return;const{data:students}=await supabase.from("students").select("id,name,sid,cls,sec").in("id",ids).order("name");if(live)setResolvedStudent(students?.[0]||null)}catch{}})();return()=>{live=false}},[student,canReview,user?.id,user?.role]);
 useEffect(()=>{if(!user||canReview)return;const cleanup=mountNavItem(()=>setOpen(true));return cleanup},[user,canReview]);
 if(!user)return null;
