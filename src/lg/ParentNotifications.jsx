  ...
  return <div role="dialog" aria-modal="true" aria-label="Notifications" style={{position:"fixed",inset:0,zIndex:5000,display:"flex",justifyContent:"flex-end",pointerEvents:"auto"}} onClick={onClose}>
    <div onClick={e=>e.stopPropagation()} style={{width:330,maxWidth:"100vw",background:"#fff",height:"100vh",overflowY:"auto",boxShadow:"-8px 0 40px rgba(27,16,96,.18)",position:"relative",zIndex:5001}}>
      ...
      <div style={{padding:"12px 14px"}}>
        ...
        {!loading&&!error&&notifs.map(n=><button type="button" key={n.id} onClick={()=>void markOne(n.id)} style={{width:"100%",textAlign:"left",padding:14,borderRadius:14,marginBottom:10,cursor:"pointer",background:n.read?"#fff":"#F0F4FF",border:"1px solid #EEF2FF"}}><div style={{fontSize:13,fontWeight:700,color:C.text}}>{n.title}</div><div style={{fontSize:12,color:C.sub,marginTop:3}}>{n.desc}</div><div style={{fontSize:10,color:C.sub,marginTop:6}}>{n.time}</div></button>)}
      </div>
    </div>
  </div>;
}
