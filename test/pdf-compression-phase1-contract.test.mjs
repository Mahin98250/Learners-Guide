...
const migration = fs.readFileSync("supabase/migrations/20260920120000_pdf_compression_phase1_tenancy_jobs.sql", "utf8");
const functionSource = fs.readFileSync("supabase/functions/pdf-compression-jobs/index.ts", "utf8");
...
test("Job API requires explicit user/secret auth and tenant-source binding", () => {
  ...
  assert.ok(functionSource.includes('withSupabase({ auth: ["user", "secret"] }'));
