...
assert.doesNotMatch(app,/TeacherAnalytics/);
assert.doesNotMatch(app,/key:\s*"analytics"/);
assert.doesNotMatch(app,/tab\s*===\s*"analytics"/);
...
assert.match(teacher,/\.eq\(\s*"batch_id"\s*,\s*active\.batch_id\)/);
assert.match(tests,/\.in\(\s*"batch_id"\s*,\s*ids\s*\)/);
assert.match(tests,/\.eq\(\s*"created_by"\s*,\s*teacher\.id\)/);
assert.match(materials,/\.eq\(\s*"tid"\s*,\s*teacher\.id\)/);
